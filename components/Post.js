  ...
  return (
    <div className='bg-white my-7 border rounded-sm'>
        ...
        <div className='flex items-center p-5'>
            <img className='rounded-full h-12 w-12 border p-1 mr-3 object-contain' src={userImg} alt='' />
            ...
            <div className='h-20 ml-10 overflow-y-scroll scrollbar-thumb-back scrollbar-thin'>
                {comments.map(item => (
                    <div key={item.id} className='flex items-center space-x-2 mb-3'>
                        <img className='h-7 rounded-full' src={item.data().userImage} alt='' />
                        <p className='text-sm flex-1'><span className='font-bold'>{item.data().username} </span>{item.data().comment}</p>
